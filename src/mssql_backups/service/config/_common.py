...
def required_text(value: str | None, prompt_text: str, *, hide_input: bool = False) -> str:
	candidate = value
...
def print_backups(backups: list[Backup]) -> None:
	...
	for backup in backups:
		description = f" | description={backup.description}" if backup.description else ""
		container = "si" if backup.is_container else "no"
		console.print(
			f"- {backup.name}{description} | backup_dir={backup.backup_dir} | data_dir={backup.data_dir} | is_container={container}"
		)