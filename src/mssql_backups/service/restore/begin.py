...
def begin(
	conn: str | None = typer.Option(None, "--conn", help="Nombre de la conexión guardada"),
	bak: str | None = typer.Option(None, "--bak", help="Nombre de la configuración de backup"),
) -> None:
	...
	with session_scope() as session:
		...
		if backup is None:
			console.print(f"[red]No existe una configuración de backup llamada {backup_name}[/]")
			raise typer.Exit(code=1)
	...
	for file_name in files:
		...
		query = build_restore_query(db_config, backup_path, backup_config.data_dir, db_name)
